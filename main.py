...
def batchify(data, bsz):
    ...
    data = data.view(bsz, -1).t().contiguous()          # .t(),求矩阵转置
...
def get_batch(source, i):
    data = source[i:i+1]
    target = source[i+1:i+1+1].view(-1)
    return data, target
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1)):  # 由于我们已经把train_data整理成batch构成的数组了，这里直接进行迭代即可。
